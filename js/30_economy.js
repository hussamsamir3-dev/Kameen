...
{ const fin = CP.Career.finishShift; CP.Career.finishShift = function () { const r = fin.apply(this, arguments); try { const G = CP.G, s = G.shift; if (G.mode !== 'career' && s && s.prog) { const k = CP.SAVE_KEY; CP.SAVE_KEY = 'cpns_save_v1'; const car = CP.loadSaved(); if (car && car.career) { car.career.xp = (car.career.xp || 0) + (s.prog.xp || 0); car.career.egp = (car.career.egp || 0) + (s.pay ? s.pay.total : 0); car.career.mail = car.career.mail || { cmp: 0, cmd: 0 }; CP.Prog.ensure(car.career); const keep = CP.G; CP.G = car; CP.save(); CP.G = keep; CP.UI.toast(CP.t('ec_merged'), 'ok'); } CP.SAVE_KEY = k; } } catch (e) { console.warn('merge', e); } return r; }; }
...
EC.pay = function (s, rep) {
  ...
  const total = rows.reduce((a, r) => a + r[1], 0); s.pay = { rows, total }; if (CP.G.mode === 'career') car.egp = EC.wallet(car) + total; return s.pay;
};
...
{ const rep = CP.Screens.report; CP.Screens.report = function (r) { const s = CP.G.shift; const pay = s ? EC.pay(s, r) : null; if (s) { s.events.coffee = false; if (CP.G.mode === 'career') CP.save('auto'); } const out = rep.apply(this, arguments); if (!pay) return out;
...
EC.shop = function () {
  ...
  const item = (k, it) => { const own = !!car.gear[k], ok = ri >= it.rank, can = EC.wallet(car) >= it.price; const icon = { leds: '💡', cam: '📷', radio: '📻', ac: '❄️', coffee: '☕', trim: '✨' }[k];
    return eh('button', { class: 'perk shop' + (own ? ' own' : ok ? '' : ' locked'), onclick: () => { if (own && !it.consumable) return; if (!ok) { CP.UI.toast(CP.t('ec_rank', { r: CP.Prog.rankName(it.rank) }), 'warn'); return; } if (!can) { CP.UI.toast(CP.t('ec_poor'), 'warn'); CP.Audio.chime('bad'); return; } car.egp -= it.price; car.gear[k] = it.consumable ? (car.gear[k] || 0) + 1 : true; CP.save(); CP.Audio.fanfare(); CP.Polish.burst && CP.Polish.burst(wrap.querySelector('.wallet')); close(); EC.shop(); } },
  ...
  wrap.appendChild(eh('div', { class: 'mc glass perks' }, eh('div', { class: 'row' }, eh('b', { style: 'font-size:1.2em' }, CP.t('ec_shop')), eh('span', { class: 'sp' }), eh('span', { class: 'tag ok wallet' }, '💵 ' + EC.fmt(EC.wallet(car))), eh('button', { class: 'btn sm ghost', onclick: close }, '✕')), eh('div', { class: 'small muted' }, CP.t('ec_shopSub')), eh('div', { class: 'perk-grid two' }, eh('div', { class: 'perk-branch' }, ...['radio', 'leds', 'ac'].map(k => item(k, EC.ITEMS[k]))), eh('div', { class: 'perk-branch' }, ...['coffee', 'cam', 'trim'].map(k => item(k, EC.ITEMS[k]))))));
...
;(window.CP_FILES = window.CP_FILES || {})['30_economy'] = '2.1.0';
